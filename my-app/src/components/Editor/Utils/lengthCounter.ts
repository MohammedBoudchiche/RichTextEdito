...
export function findParentBlockElement():string|null {
 
  const select = getTextSelection();
  if (!select) {
    console.log('No text selection available');
    return null;
  }
  
  // Now safe to destructure since we checked for null
  ...
  let counter =cursorPosition
  let previous = currentNode.previousSibling
  while(currentNode) {

    do
    {
         counter += getTextNodeLength(previous)
    }while((previous?.previousSibling))

     if(previous?.parentNode!== null){
        previous = previous?.parentNode.lastChild || null
        if(previous===null) return counter
     }    


    if(currentNode instanceof HTMLElement && currentNode.dataset.id){
      console.log('here current node parent id',currentNode.dataset.id)
      return  currentNode.dataset.id
